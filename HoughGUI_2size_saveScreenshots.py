...
def HoughTestGUI_window():
    ...
    button_HoughTestGUI_1 = tk.Button(root1, text='STEP 2A: Hough Detection Preview (BOTH SIZES)',
                                      command=HoughTestGUI_detectionPreview, font=buttonsFont)
    button_HoughTestGUI_1.grid(column=0, row=1)

    button_HoughTestGUI_2 = tk.Button(root1, text='STEP 3: Save Screenshot',
                                      command=HoughTestGUI_saveScreenshot, font=buttonsFont)
    button_HoughTestGUI_2.grid(column=0, row=2)
    ...
    text2.grid(column=0, row=10)

    global canny_2
    canny_2 = tk.IntVar()
    canny_2 = tk.Scale(root1, from_=1, to=200, orient='horizontal')
    canny_2.set(33)
    canny_2_label = ttk.Label(root1, text='Canny Edge Detector (Param 1)')
    canny_2.grid(column=0, row=11)
    canny_2_label.grid(column=0, row=12)

    global center_2
    center_2 = tk.IntVar()
    center_2 = tk.Scale(root1, from_=1, to=250, orient='horizontal')
    center_2.set(18)
    center_2_label = ttk.Label(root1, text='Center Detection Threshold (Param 2)')
    center_2.grid(column=1, row=11)
    center_2_label.grid(column=1, row=12)
    ...
    minimum_radius_2.grid(column=0, row=13)
    minimum_radius_2_label.grid(column=0, row=14)
    ...
    maximum_radius_2.grid(column=1, row=13)
    maximum_radius_2_label.grid(column=1, row=14)
    ...
    minimum_distance_2.grid(column=0, row=15)
    minimum_distance_2_label.grid(column=0, row=16)
    ...
    blur_level_2.grid(column=1, row=15)
    blur_level_2_label.grid(column=1, row=16)
    ...
    text4 = Label(root1, text='Screenshots are saved as original file name + "houghGUIanalysis" + #s of each bead type', font=buttonsFont)
...
def HoughTestGUI_detectionPreview():
    ...
    print("Red circles found, before duplicate check:" + str(circles_found1a))
    ...
    if circles2 is not None:
        circles_found2a = len(circles2[0, :])
    else:
        circles_found2a = 0

    print("Green circles found, before duplicate/overlap check:" + str(circles_found2a))

    circles2_delete_list = []

    if circles2 is not None:

        for idx_c, k in enumerate(circles2[0]):
            x_c = int(k[0])
            y_c = int(k[1])
            radius_c = int(k[2])

            for idx_d, l in enumerate(circles2[0][idx_c + 1:], start=idx_c + 1):
                x_d = int(l[0])
                y_d = int(l[1])
                radius_d = int(l[2])

                inequality_left = (((x_d - x_c) ** 2) + ((y_d - y_c) ** 2))
                inequality_right = ((radius_c) ** 2)

                if inequality_left < inequality_right:
                    circles2_delete_list.append(idx_d)

        circles2_delete_list_refined = np.unique(circles2_delete_list)
        circles2_delete_list_refined = np.array(circles2_delete_list_refined, dtype=int)
        print("Circles to delete:" + str(len(circles2_delete_list_refined)))

        if circles2_delete_list_refined is not None:
            circles2_del = np.delete(circles2, circles2_delete_list_refined, axis=1)
        else:
            circles2_del = None


    print("NOTE: Duplicate detection and overlap removal does not account for A) two actually overlapping beads which might be distinguishable by a human, or B) possible overlaps of different bead sizes -- red and green circles may still erroneously overlap.")
    ...
    if circles1_del is not None:
        ...
        print("'Smaller'/RED Parameter Set, after duplicate/overlap check: " + str(circles1_count) + " circles detected")
    else:
        print("'Smaller'/RED Parameter Set: No circles detected\n\n\n")

    if circles2_del is not None:
        circles2 = np.uint16(np.around(circles2_del))
        for i in circles2[0]:
            circles2_count += 1
            center = (i[0], i[1])
            # Circle center
        ...
        print("'Larger'/GREEN Parameter Set, after duplicate/overlap check: " + str(circles2_count) + " circles detected")
    else:
        print("'Larger'/GREEN Parameter Set: No circles detected\n\n\n")

    cv.imshow("detected circles", img)

def HoughTestGUI_saveScreenshot():
    global img

    base_name = os.path.splitext(os.path.basename(tester_image_name))[0]
    default_name = f"{base_name}_houghGUIanalysis_"+str(circles1_count)+"x3um_beads+"+str(circles2_count)+"x5um_beads.png"
